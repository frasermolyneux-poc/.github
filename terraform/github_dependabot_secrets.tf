resource "github_dependabot_secret" "client_id" {
  for_each = { for each in local.project_environments : each.key => each if each.key == "poc" }
...
resource "github_dependabot_secret" "tf_backend_subscription_id" {
  for_each = { for each in local.project_environments : each.key => each if each.configure_terraform_state == true && each.key == "poc" }
